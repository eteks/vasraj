...
<html lang="en">
    <head>
        ...
        <meta name="description" content="The photo gallery contains the several actions,meetings and tasks performed by the puducherry governor such as 10 AM meetings,Department Meetings,Meetings with Dignitaries,Other Engagements,Surprise Visits,Events at RajNivas,Open House,Public Appointments,Regional Visits,Temple Visits,Functions attended in States,Visitors to Rajnivas,Visit to Village Tanks,Budget Speech">
        ...
        <section class="boxed-wrapper page">
            ...
            <section class="maincontent" id="maincontent">
                ...
                <section class="container" id="maincontentbelow">
                    <div class="row">
                        <div class="divide10"></div>
                        <div class="span12"><h3 class="article-title">Photo Gallery</h3></div>
                        <div class="divide5"></div>
                    </div>
                    <div class="row">
                        <article>
                                ...
                                <section class="portfolio-section port-col">
                                    <div class="row">
                                        <div class="isotopeContainer">
                                            <?php
                                            $stmt = $db->prepare("select a.id, a.cat_name, SUBSTRING_INDEX(group_concat(b.image order by b.id ASC SEPARATOR '**'),'**',-1) from category a inner join subcategory sub on a.id=sub.cat_id inner join gallery b on sub.id=b.category_id group by a.id ORDER BY id DESC");
                                            $stmt->execute();
                                            $stmt->store_result();
                                            $stmt->bind_result($pk, $c_name, $g_image);
                                            if ($stmt->num_rows >= 0) {
                                                while ($stmt->fetch()) {
                                                    ?>          
                                                    <div class="span3 isotopeSelector">
                                                        <article class="">
                                                            <a href="gallerysub.php?id=<?= base64_encode($pk); ?>" >
                                                                <figure>
                                                                    <img src="admin/gallery/<?php echo $g_image; ?>" alt="">
                                                                </figure>
                                                                <div class="article-title"><?php echo $c_name; ?></div>
                                                            </a>
                                                        </article>
                                                    </div>
                                                <?php }
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-97471949-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-97471949-1');
</script>
    </body>
</html>
