        ...
        <section class="boxed-wrapper page">
            ...
            <section class="maincontent" id="maincontent">
                ...
                <section class="container" id="maincontentbelow">
                    <div class="row">
                        <?php
                            $catids = base64_decode($_REQUEST['id']);
                            $stmt = $db->prepare("select cat_name from category where id=$catids");
                            $stmt->execute();
                            $stmt->store_result();
                        ...
                        <div class="span2"><h4><a href="gallery.php"><< Back to Gallery</a></h4></div>
                        <div class="divide10"></div>
                    </div>
                    <div class="row">
                        <article>
                                ...
                                <section class="portfolio-section port-col">
                                    <div class="row">
                                        <div class="isotopeContainer">
                                            <?php
                                            $catids = base64_decode($_REQUEST['id']);
                                            $stmt = $db->prepare("select a.id, a.subcat_name, b.image from subcategory a left join gallery b on a.id=b.category_id where cat_id=$catids group by a.subcat_name ORDER BY a.id DESC");
                                            $stmt->execute();
                                            $stmt->store_result();
                                            ...
                                            if ($stmt->num_rows >= 0) {
                                                while ($stmt->fetch()) {
                                                    ...
                                                    <div class="span3 isotopeSelector">
                                                        <article class="">
                                                            <a href="gallerydtls.php?id=<?= base64_encode($pk); ?>&cid=<?= $_REQUEST['id']; ?>" >
                                                                <figure>
