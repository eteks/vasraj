    ...
    <body class="fixed-left">
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-sm-12">
                                ...
                                <h4 class="page-title">News Stream</h4>
                                <ol class="breadcrumb">
                                    <li>
                                        <a href="dashboard.php">Dashboard</a>
                                    </li>
                                    <li>
                                        <a href="#">News Stream</a>
                                    </li>
                                    <li class="active">
                                        View
                                    </li>
                            ...
                            <div class="col-sm-12">
                                <div class="card-box table-responsive">
                                    <table id="datatable" class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            <?php
                                            $stmt = $db->prepare("select id, n_name, n_image, ndate from newsroom ORDER BY id DESC");
                                            $stmt->execute();
                                            $stmt->store_result();
                                            $stmt->bind_result($id, $n_name, $n_image, $ndate);
                                            if ($stmt->num_rows > 0) {
                                                while ($stmt->fetch()) {
                                                    $id = base64_encode($id);
                                                    $fileext = strtoupper(end(explode('.', $n_image)));
                                                    $fileType = array('PDF' => 'fa-file-pdf-o', 'DOC' => 'fa-file-word-o', 'JPG' => 'fa-file-image-o', 'PNG' => 'fa-file-image-o');
                                                    echo '<tr>' .
                                                    '<td>' . $n_name . '</td>' .
                                                    '<td><a href="./newsimages/'.$n_image.'" target="_blank"><i style="font-size:25px;" class="fa '.$fileType[$fileext].'"></i></a></td>' .
                                                    '<td>' . date('d-m-Y', strtotime($ndate)) . '</td>' .
                                                    '<td class="actions">'
                                                    . '<a href="javascript:void(0);" onclick="editrow(\''.$id.'\');" class="on-default edit-row" title="Edit" data-toggle="modal" data-target="#con-close-modal"><i class="fa fa-pencil"></i></a>&nbsp;&nbsp;&nbsp;
                                                        <a href="javascript:void(0);" onclick="deleterow(\''.$id.'\');" class="on-default remove-row" title="Delete"><i class="fa fa-trash-o"></i></a>'
                                                    . '</td>' .
                                                    '<td style="display:none;">' . strtotime($ndate) . '</td>' .
                                                    '</tr>';
                                                }
                                            }
                                            ?>
                ...
                <div id="con-close-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
                    <div class="modal-dialog"> 
                        <div class="modal-content">
                            <form action="#" data-parsley-validate novalidate id="form-newsstream">
                                <input type="hidden" name="action" value="newsstream_save" />
                                <input type="hidden" name="pk" value="" />
                                <div class="modal-header"> 
                                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button> 
                                    <h4 class="modal-title">Add News Stream</h4> 
                                </div> 
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                ...
                $('#form-newsstream').parsley();
                
                $('#mediafileupload').on('change', fileUpload);
                $('#form-newsstream').submit(function(){
                    $.ajax({
                        type: "POST",
                        url: "includes/helper.php",
                        data: $('#form-newsstream').serialize(),
                        cache: false,
                        dataType: "JSON",
                        beforeSend: function() {
                            $('.close').trigger('click');
                ...
                $('#btnaddnew').on('click', function(){
                    $('#form-newsstream')[0].reset();
                    $('input[name=action]').val('newsstream_save');
                    $('input[name=pk]').val('');
                    $('div.modal-header h4.modal-title').html('Add News Stream');
            ...
            function editrow(idarg){
                ...
                $.ajax({
                    type: "POST",
                    url: "includes/helper.php",
                    data: 'action=newsstream_edit&id='+id,
                    cache: false,
                    dataType: "JSON",
                    beforeSend: function() {
                        $('#form-newsstream')[0].reset();
                        $('input[name=pk]').val('');
                        $('div.modal-header h4.modal-title').html('Edit News Stream');
                    },
                    success: function(data) {
                        if(data.status == '1') {
                            $('input[name=action]').val('newsstream_update');
                            ...
                            if(data.ext == '1')
                                $('#mediafileimg').attr({'src': './newsimages/'+data.pdf, 'style' : 'width:100%; height:100%;'});
                            else
                                $('#mediafileimg').attr({'src': '', 'style' : 'display:none;'});
                ...
                swal({
                    ...
                    closeOnConfirm: false
                     ...
                     $.ajax({
                        type: "POST",
                        url: "includes/helper.php",
                        data: 'action=newsstream_delete&id='+idarg,
                        cache: false,
                        success: function() {
                            location.reload();   
                        }
            ...
            function fileUpload(event){
                ...
                xhr.open('POST', 'includes/helper.php?action=mediacenter_upload_file&dest=newsimages', true);  
                xhr.send(data);
                xhr.onload = function () {
                    ...
                    if(xhr.status === 200 && response.status == 1){
                        ...
                        $('.text-danger').hide();
                        if(response.ext == '1')
                            $('#mediafileimg').attr({'src': './newsimages/'+response.file, 'style' : 'width:100%; height:100%;'});
                        else
                            $('#mediafileimg').attr({'src': '', 'style' : 'display:none;'});
