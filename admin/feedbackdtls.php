...
if(isset($_REQUEST['id']) && !empty($_REQUEST['id']))
{
    ...
    $stmt = $db->prepare("select feedbackdtls_pk, fbd_name, fbd_email, fbd_address, fbd_desc, fbd_createdon from feedbackdtls_tbl where feedbackdtls_pk=?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($pk, $name, $email, $address, $desc, $createdOn);
    if ($stmt->num_rows > 0) {
        $stmt->fetch();
        $stmtupdate = $db->prepare("update feedbackdtls_tbl set fbd_readstatus='R' where feedbackdtls_pk=?");
...
<html xmlns="http://www.w3.org/1999/xhtml" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; box-sizing: border-box; font-size: 14px; margin: 0;">
    <head>
        ...
        <title>Raj Nivas - Feedback Details Print</title>
        <style type="text/css">
            img {
                max-width: 100%;
            }
