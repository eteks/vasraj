...
if(isset($_REQUEST['id']) && !empty($_REQUEST['id']))
{
    ...
    $stmt = $db->prepare("select shramdaandtls_Pk, sd_name, CASE WHEN sd_gender='M' THEN 'Male' WHEN sd_gender='F' THEN 'Female' WHEN sd_gender='T' THEN 'Transgender' END, sd_mobile, sd_email, sd_address, CASE WHEN sd_type='SP' THEN 'Swachh Puducherry' WHEN sd_type='E' THEN 'Education' WHEN sd_type='IT' THEN 'IT Support' WHEN sd_type='MS' THEN 'Media Support' WHEN sd_type='AP' THEN 'Awareness Programe' END, sd_desc, sd_createdon from shramdaandtls_tbl where shramdaandtls_pk=?");
    $stmt->bind_param('i', $id);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($pk, $name, $gender, $mobile, $email, $address, $type, $desc, $createdOn);
    if ($stmt->num_rows > 0) {
        $stmt->fetch();
        $stmtupdate = $db->prepare("update shramdaandtls_tbl set sd_readstatus='R' where shramdaandtls_pk=?");
