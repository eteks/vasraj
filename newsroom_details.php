                    ...
                    <div class="row">
                        ...
                        <div class="span12">
                            <ul class="newroomlist">
                                <li><a href="newsroom.php" class="active"><img src='images/news.png' style="width:32px;"/> News Stream</a></li>
                                ...
                                <li><a href="archives.php"><img src='images/archive.png' style="width:32px;"/> Archives</a></li>
                            </ul>
                        </div>
                        <div class="span12">
                            <h4 class="article-title" style="font-size:22px;">News Stream</h4>
                        </div>
                    </div>
                    <div class="row">
                        <article>
                            <?php
                                $r = base64_decode($_REQUEST["news"]);
                                $stmt = $db->prepare("select n_name, n_image, n_description, ndate from newsroom where id=?");
                                $stmt->bind_param('s', $r);
                                $stmt->execute();
                                $stmt->store_result();
                                $stmt->bind_result($n_name, $n_img, $n_desp, $n_date);
                                if ($stmt->num_rows > 0){
                                    while ($stmt->fetch()) {
                                        $new_desp = substr($n_desp, 150);
                                        $lg_image = "<img src='admin/newsimages/$n_img'/><br />"; ?>
                                        <div class="span12 newsroom newsdetails">
                                            <div class="span12 padding0" id="newsdesp">
                                                <?php echo $lg_image; ?>
                                                <div class="divide10"></div>
                                                <h4 class="blue-text"><?php echo $n_name; ?></h4>
                                                <div class="divide5"></div>
                                                <small><?php echo date('d-m-Y', strtotime($n_date)); ?></small>
                                                <div class="divide10"></div>
                                                <div class="news-desp"><?php echo $n_desp; ?></div>
                                                <div class="divide10"></div>
                                                <a href="newsroom.php" class="btn readmore">Back</a>
                                            </div>
                                        </div>
                                    <?php }
                                }
